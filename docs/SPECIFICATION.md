...
- ダイヤルの局名（`current_station` とスナップショットの `station`）は
  ...
  キャッシュします。任意の周波数を同じ規則で名付けるのが
  `station_at(freq_hz)` で（`current_station` はその特別な場合）、録音
  タブが使います（3.14.2）。命名ビューは再束縛 1 回で差し替わるので、
  どのスレッドから呼んでも安全です
...
| ダイヤルの局名（`nearest` / `current_station` / `station_at` / スナップショットの `station`） | その地域に限定 |
...
読み、結果を signal で返します。局名（`station_at`）もそのスレッドで
付けます。1 回の検索で命名ビューをなめるので、1 回の読み取りの中では
周波数ごとに 1 回だけ引きます。
...
| Station | `station_at` の名前。いまの命名ビューで引くので、録音後に地域を変えていれば新しい地域での名前 |
...
empty も complete なので既定の一覧に出ます。ただし「yes」とは言いません。
実ディレクトリの complete 30 件のうち 5 件がこれでした。行のツールチップ
