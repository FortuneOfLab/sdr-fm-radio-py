...
読み出しは**例外を投げません**。開けない・JSON でない・JSON だがオブジェ
クトでないサイドカーは、理由を `problem` に入れた 1 行として返り、同じ
ディレクトリの他の行を道連れにしません。パースできたものは `problem`
が空文字で、知らないキーは無視、欲しいキーが無ければ該当フィールドが
`None` です。型が違う値も `None` で、ブール値は数値として採りません。
JSON が許す `NaN` / `Infinity` も `None` です（周波数でもゲインでもなく、
`int(inf)` は `OverflowError` を投げるため — 投げない約束の関数から
例外が出ます）。
...
ドロップ数は `dropped_blocks`（IQ）と
`dropped_chunks`（audio）のうち先に見つかった方です（両方を書く経路は
ありません）。
