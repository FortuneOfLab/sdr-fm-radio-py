...
4. `[[station]]` を最後に追加し、同一キーの既存エントリを置換します
   （`hidden` の直後に同じキーを再追加することも可能）。
...
判定対象がマージ結果である点は、TOML の記述順で結果が変わりうることを
意味します。`[[override]]` で `favorite = true` を付けた局を、後続の
`[[station]]` が同一キー（丸めた周波数と送信所）で置換すると、その指定は
置換された局ごと消え、既定 10 波が適用されます。
...
| `[[station]]` / `[[override]]` の個々の要素が不正 | その要素のみ無視 |
