...
6. I/Q 位相補正 — **ゲート付き4象限トラッカー**: 主軸推定
   ...
   仮定。
   **どのゲートが閉じるか**（2026-09-21 実測）: ほぼモノラルの番組で
   閉じるのは (b) の side/mono ゲートであり、(c) ではない。意図的に
   30°/60° の回転を与えた合成信号で、side/mono が −20 dB までは
   トラッカーが回転を完全に取り除き、−25/−30 dB では (b) が閉じて
   トリム事前値 0 を保持する。(c) は無音時のノイズ疑似軸に対する
   マージン（無音実測 CNR 45/35/25/15 で med 22.6 / max 24.8 dB、
   ゲートまで 1.2 dB）であり、参照録音 4 本では (c) を 6 dB まで
   開いても音声差は −58〜−70 dB。唯一の例外が番組の狭い
   optical 82.5（side/mono med −14.5 dB）で、開くとトラッカーが
   −3.4° から −17.6° へ流れる。したがって「クリーンな局で取得
   しない」ことは (c) の較正が高すぎる証拠にはならない。
