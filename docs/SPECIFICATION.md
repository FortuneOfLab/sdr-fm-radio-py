...
- **テレメトリ**: 処理スレッドが immutable なスナップショットを単一
  スロットへ書き込むだけ（GIL 下でアトミック）。ロックを取らないため
  読み手が止まっても処理スレッドは待たされず、古いスナップショットは
  キューに積まず捨てる。公開レートは 20 Hz に制限し、対象外のブロック
  の追加コストは比較 1 回のみ。
...
選局時は `telemetry.reset()` で前局のスナップショットを破棄します。
...
- `TelemetryPublisher`: 単一writer・複数readerの最新値スロット。
  `due()` → `publish()` → `latest` の 3 メソッドのみ。
