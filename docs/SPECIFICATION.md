...
| 軽量（Light） | 250 kHz | 位相差分 | 低 CPU 負荷 |
...
- 生 IQ 録音は `IQRecordWorker` へ委譲（3.10 参照）: flush sentinel、
  4 GiB ローテーション、書き込み例外の握り込み、メタデータサイドカー
...
`process_iq_samples()` は位相差分（`angle→unwrap→diff`）で FM 復調し、
`StatefulResampler` 96:125 で 250 kHz→192 kHz に変換、
`LIGHT_COMPOSITE_SCALE=0.35` を乗じます。以降のステレオ復調は標準モード
と共通（フィルタ次数のみ order-1）。
...
`SideNoiseReducer`（3.6 参照）が音声出力段で mid=(L+R)/2 を無加工の
...
- 録音は `AudioRecordWorker` へ委譲（3.10 参照）
- `record()` は bounded queue への投入のみで即時復帰
- start / stop / rotation / sidecar は 3.10 と共通の設計
...
IQ (250 kHz)
  → DC 除去
  → 位相抽出 (np.angle) → unwrap (ブロック連続) → 差分 (np.diff)
