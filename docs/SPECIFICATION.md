...
開けなかったときの扱いは 2 通りです。**stat と open の間に消えていた**
（`FileNotFoundError` / `NotADirectoryError`）なら `missing` です — 返す
時点で無いものを「在る」と言わないため。**在るが開かせてもらえない**
（権限、切れた共有など、その他の `OSError`）なら「在るが識別できない」
で、下の inode の制限と同じ経路に入ります。`close()` が失敗した場合は
無視します（読むためだけに開いたハンドルで失う書き込みは無く、答えは
close の前に出ています。ネットワークファイルシステムはここで失敗する
ことがあります）。
