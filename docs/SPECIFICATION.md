...
**NR オフ（`side_nr_enabled = False`）はバイパスであってスキップでは
ありません**: テイルは連鎖に残ったまま `bypass=True`（ユニティ OLA、
FFT なし、ゲイン状態は凍結）で駆動されます。スキップすると (a) 保持中の
side がテイル内に残り、再有効化時に再生され（実測: 無音ブロックに対し
mean |side| 0.1875、peak 1.0）、(b) 16 ms のレイテンシが出入りして
タイムラインが跳ねます — mono↔stereo で一度直したのと同じ欠陥です。
バイパスなら on/off でレイテンシもサンプル収支も変わらないため、
GUI から無クリックで A/B できます。費用は 16 ms ブロックあたり 83 us
（NR 動作時は 867 us）。テイルが透明な設定（alpha_floor=1, beta=0）で
off→on を跨いだ出力は、一度も切り替えない出力と同じく入力との差
4.8e-07（スキップ実装では 5.21）。
