...
選局時は `telemetry.invalidate()` で世代を進め、前局のスナップショット
（および処理中ブロックがこれから公開するもの）を無効化します。
...
- `TelemetryPublisher`: 最新値スロット。スナップショットの writer は処理
  スレッドのみですが、`invalidate()` は選局したスレッドから呼ばれるため、
  公開と無効化は競合します。スナップショットに**世代**を付け、
  公開時の世代が現在と異なれば `latest` は `None` を返すことで、
  「選局直前に生成し直後に公開されたスナップショット」が復活しません。
- 公開できなかった場合は `defer()` で次回期限だけを進めます。これがないと
  失敗が毎ブロック再試行され、1つの不具合がブロックごとの警告になります。
  警告自体も 60 秒に 1 回へ制限しています。
