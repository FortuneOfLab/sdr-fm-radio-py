...
- JSON だがオブジェクトでない
- `parts` に**ファイル名でない要素**が 1 つでもある（下記）
- 正規化後のパート名が**重複**している（下記）

パースできたものは `problem` が空文字で、知らないキーは無視、欲しい
キーが無ければ該当フィールドが `None` です。数値フィールドが採るのは
**有限の数に変換できる値**で、JSON の文字列 `"8.7"` は `8.7` になります。
`None` になるのは変換できない値（`"forty-eight"`、`null`、オブジェクト）、
...
`parts` の中身が信用できない 2 つの形は、行ごと `problem` にします。
**名前でない要素が混ざっている**場合、その要素だけを捨てて残りを採ると、
残った名前は全部存在するので「完全・長さ測定済み」になってしまいます
（サイドカーは「もう 1 つパートがあった」と言っているのに）。**正規化後
に同じ名前が 2 回**出る場合（`["x/a.wav", "y/a.wav"]`）は、1 つのファイル
を 2 回測って 1 秒の録音が 2 秒になります。書き手はどちらも生成しません。
