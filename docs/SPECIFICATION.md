...
`complete` が真。`missing` は `problem` が出ても**最後まで集めます**
（何が再生できるかを決めるのは `missing` なので、問題の手前で打ち切ると
「欠けていない」と言ってしまいます）。

`os.stat` は symlink を**辿ります**。隣に置かれた symlink は、参照先が
どこにあってもそれが測定対象です。意図的で、「隣を探す」という制限は
*サイドカーの書いた文字列*に連れて行かれないためのものであり、ディスク
の持ち主が自分で張ったリンクを拒むためのものではありません（4 GB の
キャプチャを別ドライブへ移して symlink を残す、という運用を壊さない）。
...
（`problem` にはしません。ファイルは揃っているので `complete` は真の
ままで、出さないのは長さだけです）。パートが 1 個なら自分自身と重複
しようがないので測ります。
