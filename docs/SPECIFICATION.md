...
- 開けない（`OSError`）
- JSON として読めない — `ValueError`（`JSONDecodeError` と
  `UnicodeDecodeError` のほか、CPython が 4300 桁超の数値に投げる桁数
  制限も含む）と `RecursionError`（深く入れ子になった有効な JSON）。
  `BaseException`（`KeyboardInterrupt` など）は捕まえません
- JSON だがオブジェクトでない
