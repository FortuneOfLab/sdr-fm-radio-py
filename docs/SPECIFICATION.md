...
6. I/Q 位相補正 — **ゲート付き4象限トラッカー**: 主軸推定
   ...
   仮定。
   **どのゲートが閉じるか**（2026-09-21 実測）: 良好な信号でほぼ
   モノラルの番組に対して閉じるのは (b) の side/mono ゲートであり、
   (c) ではない。意図的に 30°/60° の回転を与えた合成信号で、各ゲートを
   通過するブロックの割合は次のとおり（公称はプリエンファシス前の
   side/mono、実測が (b) の比較対象）:

   | 公称 | 実測 side/mono | (b) 通過 | (c) 通過 | 取得 |
   |---|---|---|---|---|
   | −10 dB | −9.4 dB | 100.0% | 100.0% | する |
   | −20 dB | −18.5 dB | 29.9% | 99.4% | する |
   | −25 dB | −23.0 dB | 0.0% | 94.9% | しない |
   | −30 dB / CNR 20 | −22.8 dB | 0.0% | 7.6% | しない |

   −25 dB では (b) だけが閉じている。信号自体が弱いときに限り両者が
   閉じる。−20 dB までは回転を完全に取り除くが、収束は遅い（広い番組
   0.1 s に対し 1.8 s）。それ以下では事前値 0 を保持し、その代償は
   事前値と真値の差（参照録音 4 本で ±7° 以内 = side 帯 0.06 dB）。

   (c) は無音時のノイズ疑似軸に対するマージン（無音実測 CNR
   45/35/25/15 で med 22.6 / max 24.8 dB、ゲートまで 1.2 dB）。一方で
   本物だが狭い内容（side/mono −18.5 dB）は中央値 28.7 dB を読むため、
   上げる余地もほとんどない。参照録音 4 本で (c) を 6 dB まで開いた
   ときの音声差は −58.5 / −50.7 / −69.6 / −59.2 dB、角度差は 3 本で
   5.4° 以内。4 本目が番組の狭い optical 82.5（side/mono med
   −14.5 dB）で、開くと −3.4° から −17.6° へ流れる。どちらの角度が
   真値かはこの測定では確定しないが、振れはノイズ由来の形をしている:
   ゲートを開いた状態で、番組が広い四分位では −7.2°、狭い四分位では
   −13.2°、ゲート出荷設定では −7.1° / −6.7°（番組の幅によらず同じ
   答え）。いずれにせよ音声差は −50.7 dB。したがって「クリーンな局で
   取得しない」ことは (c) の較正が高すぎる証拠にはならない。
