...
6. I/Q 位相補正 — **ゲート付き4象限トラッカー**: 主軸推定
   ...
   (a) 共分散異方性 ≥ `STEREO_PHASE_ANISO_GATE`(0.2)かつ
   (b) side 電力が mono 電力の `STEREO_PHASE_SIDE_GATE_DB`(-18 dB)
   以上、の両条件を満たす情報のあるブロックのみで、π 周期族のうち
   現追跡値に最も近い候補への差分で EMA を更新（連続性が 180° 分岐を
   解決、クランプ不要で ±180° 追従）。モノラル番組中は両ゲートが
   閉じて追跡値を凍結。コールドスタート取得は連続
