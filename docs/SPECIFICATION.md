...
6. I/Q 位相補正 — **ゲート付き4象限トラッカー**: 主軸推定
   ...
   **どのゲートが閉じるか**（2026-09-21 実測）:
   ...
   まず測定上の注意。**番組がある信号では pilot ノイズ帯の推定値が
   立ち上がるまで約 3 秒かかり**、その間 side-over-noise は 4〜5 dB
   高く出る（無音では最初のブロックから正しい）。同じ −20 dB の番組が
   最初の 2.5 秒では中央値 28.8 dB、それ以降は 25.3 dB。短い測定は
   過渡を測っている。

   合成信号（回転 30°/60°、6 秒のうち整定後のブロック）で、各ゲートを
   通過する割合:

   | 公称 | 実測 side/mono | (b) 通過 | (c) 通過 | 取得 |
   |---|---|---|---|---|
   | −10 dB | −9.2 dB | 100.0% | 100.0% | する |
   | −20 dB | −17.0 dB | 100.0% | 15.5% | する |
   | −25 dB | −21.1 dB | 0.0% | 3.7% | しない |
   | −30 dB / CNR 20 | −19.6 dB | 0.0% | 0.0% | しない |

   整定後は両者がほぼ同時に閉じる（中身のない side 帯は mono に対しても
   ノイズに対しても小さい）。ただし**コールドスタートの取得は最初の
   数秒で決まり**、その間 (c) はほぼ全通しなので、取得の可否を決めて
   いるのは (b) である（−25 dB で (b) だけを開けば取得し、(c) だけを
   開けても取得しない）。−20 dB までは回転を完全に取り除くが収束は遅い
   （広い番組 0.1 s に対し 1.8 s）。それ以下では事前値 0 を保持し、
   代償は事前値と真値の差（side 帯 0.06 dB、下記）。

   実録音でブロック単位に見ると、拒否されたブロックのうち (b) のみ /
   (c) のみ / 両方で落ちた割合は、optical 82.5 が 19 / 33 / 48%、
   CATV 83.7 が 10 / 71 / 18%、optical 80.0 が 28 / 14 / 58%、
   antenna 91.6 が 0 / 100 / 0%。**「取得しない局は (b) のせい」と
   一般化はできない**。実際の番組では (c) が同等以上に拒否している。

   (c) の較正: 下側は無音時のノイズ疑似軸に対するマージン（無音実測
   CNR 45/35/25/15 で med 22.6 / max 24.8 dB、ゲートまで 1.2 dB）。
   上側の余裕はそれより小さく、本物だが狭い番組（side/mono −17 dB、
   CNR 35）は整定後 25.6 dB とゲートを下回り、上に出る 15% の
   ブロックで取得している。同じ番組をゲート 29 dB にすると一度も
   取得しない。
