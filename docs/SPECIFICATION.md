...
パートの**同一性と長さは 1 つのハンドルから**取ります（`os.stat` では
なく、開いたうえで `os.fstat`）。別々に見ると食い違いえます — `a.wav`
を stat した後にそれが別ファイルへ差し替えられると、同一性は古い
ファイルのもの、長さは新しいファイルのものになり、1 つのファイルが
2 回測られて 1 秒の録音が 20 秒になります。開いたまま両方を取れば、
同じファイルであることが構造的に保証されます。

開けなかったときの扱いは 2 通りで、**通常ファイルがまだそこに在るか**
で分けます。無ければ `missing`（返す時点で無いものを「在る」と言わない
ため）、在れば「在るが識別できない」で、下の inode の制限と同じ経路に
入ります。

**例外の種類では分けません。** ファイルがディレクトリに差し替わった
場合、Linux は `IsADirectoryError`、**Windows は `PermissionError`** を
投げ、一方「読めないだけの通常ファイル」はどちらでも `PermissionError`
です。種類で分けると片方のプラットフォームでだけ正しくなります。そこで
`FileNotFoundError` / `NotADirectoryError` / `IsADirectoryError` は無条件
に `missing`、その他の `OSError` / `ValueError` は**もう一度 stat して**
通常ファイルが残っているかで決めます。

同じ問いを `os.fstat` にもう一度、パスではなく**ハンドル**に対して
尋ねます。測るのはハンドルなので、通常ファイルでなければならないのも
ハンドルです。

`close()` が失敗した場合は無視します（読むためだけに開いたハンドルで
失う書き込みは無く、答えは close の前に出ています。ネットワーク
ファイルシステムはここで失敗することがあります）。
