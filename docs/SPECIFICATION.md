...
- 開けたが読み取りで `OSError`（`could not be read`）
...
パートは「在るが識別できない」とし、測りません。名前に戻るのは、
...
しません。出さないのは長さだけで、`complete` は従来どおり `missing` が
空かどうかで決まります（識別できないパートが 2 個あり、さらに別の
パートが欠けていれば `complete` は偽ですが、それは欠けているからです）。
