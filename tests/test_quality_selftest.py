...
def _a_tone(seconds=3.0, hz=1000.0, fs=48_000):
    t = np.arange(int(seconds * fs)) / fs
    return 0.5 * np.sin(2 * np.pi * hz * t)


def _delayed(x, samples):
    """A reference delay, independent of the one under test."""
    n = x.size
...
def test_the_sub_sample_lag_is_found_through_a_gain(lag, gain):
    ...
    ref = _a_tone()
    found = _sub_sample_lag(ref, gain * _delayed(ref, lag))
...
def test_the_snr_metric_reports_the_noise_it_was_given():
    ...
    for hz in (250.0, 1000.0, 4000.0):
        ...
        # noise is measured against.
        carried = 0.8 * _delayed(ref, 0.23)
...
def test_a_lag_that_is_not_sub_sample_is_left_to_the_integer_step():
    ...
    ref = _a_tone()
    assert _sub_sample_lag(ref, _delayed(ref, 7.0)) == 0.0
